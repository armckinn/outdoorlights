...
"""
Wrapper around the DMX ola class. Also includes various update functions.
"""


class LinearFade(object):
    """Perform a simple linear fade for a list of LED lights. All lights gets
    same value. The fading is done based on human perception of light and
    not just evening going from low to high."""

    def __init__(self, lights, start_value, end_value, num_steps):
        """Initialization

        lights       List of positions in the DMX universe
        start_value  Starting point for all the lights
        end_value    Ending value for all the lights
        num_steps    Number of expected steps to perform the fader
        """
    ...
    def UpdateLights(self, data, cur_step):
        """This function gets called by the DMXUpdater class to perform an
        update on all the lights.
        """
